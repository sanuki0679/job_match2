...
window.addEventListener('DOMContentLoaded', function () {
    ...
    submit.addEventListener('click', function () {
        ...
        // 非同期でPOST送信するHTTP Clientライブラリのaxios
        axios.post(
        ...
        // メッセージ送信後に、メッセージ入力欄を空にする
        message.value = '';
    });
    // 発生したMessageSendイベントを受け取ったら処理をする
            ...
            newDiv.innerHTML = `
                <div class="flex items-end">
                    ...
                    <div class="flex flex-col space-y-2 text-lg max-w-lg mx-2 items-start">
                        <div>
                            <span class="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-600 text-gray-600">${e.message.message}</span>
                        </div>
                    </div>
                    <div class="text-gray-600 text-sm">${ja_date}</div>
                </div>
...
function deleteMessage(e) {
    ...
    // 非同期でDELETE送信をするHTML Clientライブラリのaxios
    axios.delete(

<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-indigo-900 shadow-md rounded-md">
        ...
        <form action="{{ route('job_offers.update', $job_offer) }}" method="post"
            class="rounded pt-3 pb-8 mb-4">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-white mb-2" for="titler">
                    タイトル
                </label>
                <input type="text" name="title"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="タイトル" value="{{ old('title', $job_offer->title) }}">
            </div>
            <div class="mb-4">
                ...
                <select name="occupation_id" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3" required>
                    ...
                    @foreach ($occupations as $occupation)
                        <option value="{{ $occupation->id }}" @if ($occupation->id == old('occupation_id', $job_offer->occupation_id)) selected @endif>{{ $occupation->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <input type="date" name="due_date"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="募集期限" value="{{ old('due_date', $job_offer->due_date) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="description" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="詳細">{{ old('description', $job_offer->description) }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-white mb-2" for="description">
                    公開状況
                ...
                @foreach (App\Models\JobOffer::STATUS_LIST as $value => $name)
                    <input type="radio" name="is_published" value="{{ $value }}" @if ($value == old('is_published', $job_offer->is_published)) checked="checked" @endif required>
